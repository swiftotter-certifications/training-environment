...
use Magento\Setup\Console\CommandList as ConsoleCommandList;
...
class Cli extends Console\Application
{
    ...
    protected function getApplicationCommands()
    {
        $commands = [];
        try {
            if (class_exists(ConsoleCommandList::class)) {
                $setupCommandList = new ApplicationCommandListProxy(ConsoleCommandList::class, $this->serviceManager);
                $commands = array_merge($commands, $setupCommandList->getCommands());
            }

            if ($this->objectManager->get(DeploymentConfig::class)->isAvailable()) {
                /** @var CommandListInterface */
                $commandList = $this->objectManager->create(CommandLoader::class);
    ...
    private function initObjectManager()
    {
        ...
        // SwiftOtter Added:
        InitializeCommand::$objectManager = $this->serviceManager;
    ...
    protected function getVendorCommands($objectManager)
    {
        ...
        foreach (\Magento\Framework\Console\CommandLocator::getCommands() as $commandListClass) {
            if (class_exists($commandListClass)) {
                $commands[] = (new ApplicationCommandListProxy($commandListClass, $objectManager))->getCommands(); // SwiftOtter Changed
//                $commands[] = $objectManager->create($commandListClass)->getCommands();
    ...
    {
        if (!($command instanceof \Symfony\Component\Console\Command\ListCommand)) {
            $application = $command->getApplication();

            try {
                $command = $this->serviceManager->get(get_class($command));
            } catch (\Throwable $ex) {
                $command = $this->objectManager->get(get_class($command));
            }
